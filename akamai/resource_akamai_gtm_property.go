...
func populatePropertyObject(d *schema.ResourceData, prop *gtmv1_3.Property) {
        ...
        if v, ok := d.GetOk("name"); ok { prop.Name = v.(string) }
	if v, ok := d.GetOk("type"); ok { prop.Type = v.(string) }
        if v, ok := d.GetOk("score_aggregation_type"); ok { prop.ScoreAggregationType = v.(string) }
	if v, ok := d.GetOk("stickiness_bonus_percent"); ok { prop.StickinessBonusPercent = v.(int) }
        if v, ok := d.GetOk("stickiness_bonus_constant"); ok { prop.StickinessBonusConstant = v.(int) }
        if v, ok := d.GetOk("health_threshold"); ok { prop.HealthThreshold = v.(int) }
	if v, ok := d.GetOk("ipv6"); ok { prop.Ipv6 = v.(bool) }
        if v, ok := d.GetOk("use_computed_targets"); ok { prop.UseComputedTargets = v.(bool) }
	if v, ok := d.GetOk("backup_ip"); ok { prop.BackupIp = v.(string) }
        if v, ok := d.GetOk("balance_by_download_score"); ok { prop.BalanceByDownloadScore = v.(bool) }
        if v, ok := d.GetOk("static_ttl"); ok { prop.StaticTTL = v.(int) }
	if v, ok := d.GetOk("unreachable_threshold"); ok { prop.UnreachableThreshold = v.(int) }
        if v, ok := d.GetOk("health_multiplier"); ok { prop.HealthMultiplier = v.(int) }
        if v, ok := d.GetOk("dynamic_ttl"); ok { prop.DynamicTTL = v.(int) }
        if v, ok := d.GetOk("max_unreachable_penalty"); ok { prop.MaxUnreachablePenalty = v.(int) }
	if v, ok := d.GetOk("map_name"); ok { prop.MapName = v.(string) }
        if v, ok := d.GetOk("handout_limit"); ok { prop.HandoutLimit = v.(int) }
	if v, ok := d.GetOk("handout_mode"); ok { prop.HandoutMode = v.(string) }
	if v, ok := d.GetOk("load_imbalance_percentage"); ok { prop.LoadImbalancePercentage = v.(float64) }
	if v, ok := d.GetOk("failover_delay"); ok { prop.FailoverDelay = v.(int) }
	if v, ok := d.GetOk("backup_cname"); ok { prop.BackupCName = v.(string) }
	if v, ok := d.GetOk("failback_delay"); ok { prop.FailbackDelay = v.(int) }
	if v, ok := d.GetOk("health_max"); ok { prop.HealthMax = v.(int) }
	if v, ok := d.GetOk("ghost_demand_reporting"); ok { prop.GhostDemandReporting = v.(bool) }
	if v, ok := d.GetOk("weighted_hash_bits_for_ipv4"); ok { prop.WeightedHashBitsForIPv4 = v.(int) }
	if v, ok := d.GetOk("weighted_hash_bits_for_ipv6"); ok { prop.WeightedHashBitsForIPv6 = v.(int) }
	if v, ok := d.GetOk("cname"); ok { prop.CName = v.(string) }
	if v, ok := d.GetOk("comments"); ok { prop.Comments = v.(string) }
