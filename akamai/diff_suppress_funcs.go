...
func suppressEquivalentJsonPendingDiffs(old, new string, d *schema.ResourceDiff) bool {
	...
	rulesOld, err := getRulesForCompDiff(d, old)
	rulesOld.Etag = ""
	jsonBody, err := jsonhooks.Marshal(rulesOld)
	if err != nil {
		return false
	...
	rulesNew, err := getRulesForCompDiff(d, new)
	rulesNew.Etag = ""
	jsonBodyNew, err := jsonhooks.Marshal(rulesNew)
	if err != nil {
		return false
...
func getRulesForCompDiff(d *schema.ResourceDiff, json string) (*papi.Rules, error) {

	property, e := getPropertyDiff(d)
	if e != nil {
		return nil, e
	}
	...
	log.Printf("[DEBUG] Unmarshal Rules from JSON")
	unmarshalRulesFromJSONCompDiff(d, json, rules)
	...
	cpCode, err := getCPCodeDiff(d, property.Contract, property.Group)
	if err != nil {
		return nil, err
	}
...
func unmarshalRulesFromJSONCompDiff(d *schema.ResourceDiff, rulesComp string, propertyRules *papi.Rules) {
	// Default Rules
	...
	rulesJSON.ForEach(func(key, value gjson.Result) bool {
		...
		if key.String() == "children" {
			...
			for _, rule := range extractRulesJSONDiff(d, childRules) {
				propertyRules.Rule.MergeChildRule(rule)
			}
		}
