...
func populateResourceObject(d *schema.ResourceData, rsrc *gtmv1_3.Resource) {
        ...
        if v, ok := d.GetOk("name"); ok { rsrc.Name = v.(string) }
	if v, ok := d.GetOk("type"); ok { rsrc.Type = v.(string) }
        if v, ok := d.GetOk("host_header"); ok { rsrc.HostHeader = v.(string) }
	if v, ok := d.GetOk("least_squares_decay"); ok { rsrc.LeastSquaresDecay = v.(int) }
        if v, ok := d.GetOk("upper_bound"); ok { rsrc.UpperBound = v.(int) }
	if v, ok := d.GetOk("description"); ok { rsrc.Description = v.(string) }
	if v, ok := d.GetOk("leader_string"); ok { rsrc.LeaderString = v.(string) }
	if v, ok := d.GetOk("constrained_property"); ok { rsrc.ConstrainedProperty = v.(string) }
	if v, ok := d.GetOk("aggregation_type"); ok { rsrc.AggregationType = v.(string) }
        if v, ok := d.GetOk("load_imbalance_percent"); ok { rsrc.LoadImbalancePercent = v.(float64) }
	if v, ok := d.GetOk("max_u_multiplicative_increment"); ok { rsrc.MaxUMultiplicativeIncrement = v.(float64) }
	if v, ok := d.GetOk("decay_rate"); ok { rsrc.DecayRate = v.(float64) }
