...
func getRules(d *schema.ResourceData, property *papi.Property, contract *papi.Contract, group *papi.Group) (*papi.Rules, error) {
	...
	if ok {
		rulesString := d.Get("rules").(string)
		log.Printf("[DEBUG] Unmarshal Rules from JSON")
		unmarshalRulesFromJSON(d, rulesString, rules)
	}
...
func unmarshalRulesFromJSON(d *schema.ResourceData, rulesComp string, propertyRules *papi.Rules) {
	// Default Rules
