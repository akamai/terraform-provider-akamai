...
func TestAkamaiRuleUpgrade_data_basic(t *testing.T) {
	...
	t.Run("match by RuleUpgrade ID", func(t *testing.T) {
		...
		err := json.Unmarshal(testutils.LoadFixtureBytes(t, "testdata/TestDSRuleUpgrade/RuleUpgrade.json"), &getRuleUpgradeResponse)
		...
		client.APPSEC.On("GetRuleUpgrade",
			testutils.MockContext,
		...
		resource.UnitTest(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: testutils.LoadFixtureString(t, "testdata/TestDSRuleUpgrade/match_by_id.tf"),
