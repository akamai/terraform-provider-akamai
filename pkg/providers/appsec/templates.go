...
func InitTemplates(otm map[string]*OutputTemplate) {
	...
	otm["rulesets"] = &OutputTemplate{TemplateName: "rulesets", TableTitle: "ID|Name(Title)", TemplateType: "TABULAR", TemplateString: "{{range .Rulesets}}{{range $index, $element := .Rules}}{{if $index}},{{end}}{{.ID}}|{{replace \",\" \"\" (substring 0 45 .Title)}}{{end}}{{end}}", }
