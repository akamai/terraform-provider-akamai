...
func InitTemplates(otm map[string]*OutputTemplate) {
	...
	otm["advancedSettingsPIILearningDS"] = &OutputTemplate{TemplateName: "advancedSettingsPIILearningDS", TableTitle: "Enable PII Learning", TemplateType: "TABULAR", TemplateString: "{{.EnablePIILearning}}"}
	...
	otm["advancedSettingsRequestBodyDS"] = &OutputTemplate{TemplateName: "advancedSettingsRequestBodyDS", TableTitle: "Request Body Inspection Limit ", TemplateType: "TABULAR", TemplateString: "{{.RequestBodyInspectionLimitInKB}}"}
	otm["advancedSettingsAsePenaltyBoxDS"] = &OutputTemplate{TemplateName: "advancedSettingsAsePenaltyBoxDS", TableTitle: "Block Duration|Attack Groups|Rules", TemplateType: "TABULAR", TemplateString: "{{.BlockDuration}}|{{range $index, $element := .QualificationExclusions.AttackGroups}}{{if $index}} {{end}}{{.}}{{end}}|{{range $index, $element := .QualificationExclusions.Rules}}{{if $index}} {{end}}{{.}}{{end}}"}
	otm["advancedSettingsRequestBodyPolicyDS"] = &OutputTemplate{TemplateName: "advancedSettingsRequestBodyPolicyDS", TableTitle: "Request Body Inspection Limit | Request Body Inspection Limit Override", TemplateType: "TABULAR", TemplateString: "{{.RequestBodyInspectionLimitInKB}}|{{.RequestBodyInspectionLimitOverride}}"}
	otm["advancedSettingsJA4FingerprintDS"] = &OutputTemplate{TemplateName: "advancedSettingsJA4FingerprintDS", TableTitle: "Header Names", TemplateType: "TABULAR", TemplateString: "{{range $index, $element := .HeaderNames}}{{if $index}},{{end}}{{.}}{{end}}"}
	...
	otm["penaltyBoxConditionsDS"] = &OutputTemplate{TemplateName: "penaltyBoxConditionsDS", TableTitle: "ConditionsOperator|Conditions", TemplateType: "TABULAR", TemplateString: "{{.ConditionOperator}}|{{range $index, $element := .Conditions}}{{if $index}},{{end}}True{{else}}False{{end}}"}
	otm["evalPenaltyBoxDS"] = &OutputTemplate{TemplateName: "EvaluationPenaltyBoxDS", TableTitle: "PenaltyBoxProtection|Action", TemplateType: "TABULAR", TemplateString: "{{.PenaltyBoxProtection}}|{{.Action}}"}
	otm["evalPenaltyBoxConditionsDS"] = &OutputTemplate{TemplateName: "evalPenaltyBoxConditionsDS", TableTitle: "ConditionsOperator|Conditions", TemplateType: "TABULAR", TemplateString: "{{.ConditionOperator}}|{{range $index, $element := .Conditions}}{{if $index}},{{end}}True{{else}}False{{end}}"}
	...
	otm["siemsettingsDS"] = &OutputTemplate{TemplateName: "siemsettingsDS", TableTitle: "Enable For All Policies|Enable Siem|Enabled Botman Siem Events|Include JA4 Fingerprint to SIEM|Siem Definition ID", TemplateType: "TABULAR", TemplateString: "{{.EnableForAllPolicies}}|{{.EnableSiem}}|{{.EnabledBotmanSiemEvents}}|{{.IncludeJA4FingerprintToSiem}}|{{.SiemDefinitionID}}"}
	...
	otm["AIRulesDS"] = &OutputTemplate{TemplateName: "AIRulesDS", TableTitle: "Rule ID|Version|Title|Risk Score Group|Action", TemplateType: "TABULAR", TemplateString: "{{range $index, $element := .}}{{if $index}},{{end}}{{.RuleID}}|{{.RuleVersion}}|{{.Title}}|{{.RiskScoreGroup}}|{{.Action}}{{end}}"}
	otm["RapidRulesWithConditionExceptionDS"] = &OutputTemplate{TemplateName: "RapidRulesWithConditionExceptionDS", TableTitle: "ID|Name|Action|Lock|Attack Group|Group Exceptions|Exceptions|Advanced Exceptions|Expired|Expire In Days", TemplateType: "TABULAR", TemplateString: "{{range $index, $element := .}}{{if $index}},{{end}}{{.ID}}|{{.Name}}|{{.Action}}|{{.Lock}}|{{.AttackGroup}}|{{with .AttackGroupException}}{{if .Exception}}True{{else}}False{{end}}{{else}}False{{end}}|{{with .ConditionException}}{{if .Exception}}True{{else}}False{{end}}{{else}}False{{end}}|{{with .ConditionException}}{{if .AdvancedExceptionsList}}True{{else}}False{{end}}{{else}}False{{end}}|{{with .Expired}}{{if .}}True{{else}}False{{end}}{{else}}{{end}}|{{with .ExpireInDays}}{{.}}{{else}}{{end}}{{end}}"}
	...
	otm["protections"] = &OutputTemplate{TemplateName: "Security Policy Protections", TableTitle: "AccountProtectionControls|APIConstraints|ApplicationLayerControls|BotmanControls|MalwareControls|NetworkLayerControls|RateControls|ReputationControls|SlowPostControls|UrlProtectionControls", TemplateType: "TABULAR", TemplateString: "{{.ApplyAccountProtectionControls}}|{{.ApplyAPIConstraints}}|{{.ApplyApplicationLayerControls}}|{{.ApplyBotmanControls}}|{{.ApplyMalwareControls}}|{{.ApplyNetworkLayerControls}}|{{.ApplyRateControls}}|{{.ApplyReputationControls}}|{{.ApplySlowPostControls}}|{{.ApplyURLProtectionControls}}"}
	...
	otm["customRulesUsage"] = &OutputTemplate{TemplateName: "Custom rules usage", TableTitle: "RULE_ID|POLICY_ID|POLICY_NAME", TemplateType: "TABULAR", TemplateString: "{{range $index, $element := .}}{{if $index}},{{end}}{{.RuleID}}|{{.PolicyID}}|{{.PolicyName}}{{end}}"}
	...
	otm["AdvancedSettingsAsePenaltyBox.tf"] = &OutputTemplate{TemplateName: "AdvancedSettingsAsePenaltyBox.tf", TableTitle: "AdvancedSettingsAsePenaltyBox", TemplateType: "TERRAFORM", TemplateString: "\n// terraform import akamai_appsec_advanced_settings_ase_penalty_box.akamai_appsec_advanced_settings_ase_penalty_box {{.ConfigID}} \nresource \"akamai_appsec_advanced_settings_ase_penalty_box\" \"akamai_appsec_advanced_settings_ase_penalty_box\" { \n  config_id = {{.ConfigID}}\n  block_duration = {{.AdvancedOptions.AsePenaltyBox.BlockDuration}}\n {{with .AdvancedOptions.AsePenaltyBox.QualificationExclusions}} qualifications_exclusions {\n    attack_groups = [{{range $index, $element := .AttackGroups}}{{if $index}},{{end}}{{quote .}}{{end}}]\n    rules = [{{range $index, $element := .Rules}}{{if $index}},{{end}}{{.}}{{end}}]\n  }{{end}}\n}\n"}
	...
	otm["AdvancedSettingsJA4Fingerprint.tf"] = &OutputTemplate{TemplateName: "AdvancedSettingsJA4Fingerprint.tf", TableTitle: "AdvancedSettingsJA4Fingerprint", TemplateType: "TERRAFORM", TemplateString: "\n// terraform import akamai_appsec_advanced_settings_ja4_fingerprint.akamai_appsec_advanced_settings_ja4_fingerprint {{.ConfigID}} \nresource \"akamai_appsec_advanced_settings_ja4_fingerprint\" \"akamai_appsec_advanced_settings_ja4_fingerprint\" { \n  config_id = {{.ConfigID}} \n  header_names = [{{  range $index, $element := .AdvancedOptions.JA4Fingerprint.HeaderNames }}{{if $index}},{{end}}{{quote .}}{{end}}] \n } \n"}
	...
	otm["EvalPenaltyBox.tf"] = &OutputTemplate{TemplateName: "EvalPenaltyBox.tf", TableTitle: "EvaluationPenaltyBox", TemplateType: "TERRAFORM", TemplateString: "{{ $config := .ConfigID }}{{ $version := .Version }}{{ $prev_secpolicy := \"\" }}{{range $index, $element := .SecurityPolicies}}{{$prev_secpolicy := .ID}}{{with .EvaluationPenaltyBox}}\n// terraform import akamai_appsec_eval_penalty_box.akamai_appsec_eval_penalty_box_{{$prev_secpolicy}}{{if $index}}_{{$index}}{{end}} {{$config}}:{{$prev_secpolicy}}\nresource \"akamai_appsec_eval_penalty_box\" \"akamai_appsec_eval_penalty_box_{{$prev_secpolicy}}{{if $index}}_{{$index}}{{end}}\" { \n  config_id = {{$config}}\n  security_policy_id = \"{{$prev_secpolicy}}\" \n  penalty_box_protection =  {{.PenaltyBoxProtection}} \n  penalty_box_action = \"{{.Action}}\"   \n}\n{{end}}{{end}}"}
	otm["EvalPenaltyBoxConditions.tf"] = &OutputTemplate{TemplateName: "EvalPenaltyBoxConditions.tf", TableTitle: "EvalPenaltyBoxConditions", TemplateType: "TERRAFORM", TemplateString: "{{ $config := .ConfigID }}{{ $version := .Version }}{{ $prev_secpolicy := \"\" }}{{range $index, $element := .SecurityPolicies}}{{$prev_secpolicy := .ID}}{{with .EvalPenaltyBoxConditions}}\n// terraform import akamai_appsec_eval_penalty_box_conditions.akamai_appsec_eval_penalty_box_conditions_{{$prev_secpolicy}}{{if $index}}_{{$index}}{{end}} {{$config}}:{{$prev_secpolicy}}\nresource \"akamai_appsec_eval_penalty_box_conditions\" \"akamai_appsec_eval_penalty_box_conditions_{{$prev_secpolicy}}{{if $index}}_{{$index}}{{end}}\" { \n  config_id = {{$config}}\n  security_policy_id = \"{{$prev_secpolicy}}\" \n \n}\n{{end}}{{end}}"}
