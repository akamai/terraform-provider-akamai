...
func InitTemplates(otm map[string]*OutputTemplate) {
	...
	otm["rulesets"] = &OutputTemplate{TemplateName: "rulesets", TableTitle: "ID|Name(Title)", TemplateType: "TABULAR", TemplateString: "{{range .Rulesets}}{{range $index, $element := .Rules}}{{if $index}},{{end}}{{.ID}}|{{replace \",\" \"\" (replace \"|\" \" \" (substring 0 100 .Title))}}{{end}}{{end}}"}
	...
	otm["EvalAction.tf"] = &OutputTemplate{TemplateName: "EvalActions.tf", TableTitle: "EvalActions", TemplateType: "TERRAFORM", TemplateString: "{{ $config := .ConfigID }}{{ $version := .Version }}{{ $prev_secpolicy := \"\" }}{{range .SecurityPolicies}}{{$prev_secpolicy := .ID}} {{range $index, $element := .WebApplicationFirewall.Evaluation.RuleActions}}\nresource \"akamai_appsec_eval_rule_action\" \"akamai_appsec_eval_rule_action_{{$prev_secpolicy}}{{if $index}}_{{$index}}{{end}}\" { \n  config_id = {{$config}}\n  version = {{$version}}\n  security_policy_id = \"{{$prev_secpolicy}}\" \n  rule_id = {{.ID}} \n  rule_action = \"{{.Action}}\" \n }\n{{end}}{{end}}"}
	...
	otm["PenaltyBox.tf"] = &OutputTemplate{TemplateName: "PenaltyBox.tf", TableTitle: "PenaltyBox", TemplateType: "TERRAFORM", TemplateString: "{{ $config := .ConfigID }}{{ $version := .Version }}{{ $prev_secpolicy := \"\" }}{{range $index, $element := .SecurityPolicies}}{{$prev_secpolicy := .ID}}{{if .PenaltyBox.Action}} \nresource \"akamai_appsec_penalty_box\" \"akamai_appsec_penalty_box_{{$prev_secpolicy}}{{if $index}}_{{$index}}{{end}}\" { \n config_id = {{ $config }}\n  version = {{ $version }}\n  security_policy_id = \"{{$prev_secpolicy}}\" \n  penalty_box_protection =  \"{{.PenaltyBox.PenaltyBoxProtection}}\" \n  penalty_box_action = \"{{.PenaltyBox.Action}}\" \n}\n{{end}}{{end}}"}
