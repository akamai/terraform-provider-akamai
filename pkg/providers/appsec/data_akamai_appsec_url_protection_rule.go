...
type (
	urlProtectionRuleDataSource struct {
		meta meta.Meta
	}

	// urlProtectionRuleDataSourceModel maps the API response to a structure that can be used in the Terraform schema
	urlProtectionRuleDataSourceModel struct {
...
var (
	_ datasource.DataSource              = &urlProtectionRuleDataSource{}
	_ datasource.DataSourceWithConfigure = &urlProtectionRuleDataSource{}
)

// NewURLProtectionRuleDataSource returns a new URL protection rule data source
func NewURLProtectionRuleDataSource() datasource.DataSource { return &urlProtectionRuleDataSource{} }

// Metadata configures data source's meta information
func (d *urlProtectionRuleDataSource) Metadata(_ context.Context, _ datasource.MetadataRequest, response *datasource.MetadataResponse) {
	response.TypeName = "akamai_appsec_url_protection_rule"
}

// Schema is used to define data source's terraform schema
func (d *urlProtectionRuleDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "URL Protection Rule data source.",
...
func (d *urlProtectionRuleDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *urlProtectionRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "URLProtectionRuleDataSource Read")

	var data urlProtectionRuleDataSourceModel
	...
	getURLProtectionRuleRequest := appsec.GetURLProtectionRuleRequest{
		ConfigID:            configID,
		ConfigVersion:       int64(version),
		URLProtectionRuleID: urlProtectionID,
	}

	urlProtectionRule, err := client.GetURLProtectionRule(ctx, getURLProtectionRuleRequest)
	if err != nil {
		resp.Diagnostics.AddError("calling 'GetURLProtectionRule'", err.Error())
		return
	}

	if urlProtectionRule == nil {
		resp.Diagnostics.AddError("URL Protection Rule not found", fmt.Sprintf("URL Protection Rule with ID %d not found in config %d version %d", urlProtectionID, configID, version))
		return
	}
	...
	resp.Diagnostics.Append(resp.State.Set(ctx, &urlProtectionRuleModel)...)
	if resp.Diagnostics.HasError() {
		return
	}
}

func createURLProtectionRuleModel(ctx context.Context, response *appsec.GetURLProtectionRuleResponse) (urlProtectionRuleDataSourceModel, diag.Diagnostics) {
	var diags diag.Diagnostics
	var model urlProtectionRuleDataSourceModel

	if response == nil {
		diags.AddError("Invalid response", "GetURLProtectionRuleResponse is nil")
		return model, diags
	}

	// Set basic fields
	model.ConfigID = types.Int64Value(response.ConfigID)
	model.URLProtectionID = types.Int64Value(response.URLProtectionRuleID)
	...
	if response.APIDefinitions != nil && response.HostnamePaths != nil {
		diags.AddError("Invalid response", "URLProtectionRule cannot have both APIDefinitions and HostnamePaths defined at the same time")
		return model, diags
...
func populateBypassConditions(ctx context.Context, model *urlProtectionRuleDataSourceModel, response *appsec.GetURLProtectionRuleResponse, diags diag.Diagnostics) diag.Diagnostics {
...
func populateAPIDefinitions(ctx context.Context, model *urlProtectionRuleDataSourceModel, response *appsec.GetURLProtectionRuleResponse, diags diag.Diagnostics) diag.Diagnostics {
...
func populateHostnamePaths(ctx context.Context, model *urlProtectionRuleDataSourceModel, response *appsec.GetURLProtectionRuleResponse, diags diag.Diagnostics) diag.Diagnostics {
...
func populateCategoriesAndCustomCriteria(ctx context.Context, ilsModel *intelligentLoadSheddingModel, response *appsec.GetURLProtectionRuleResponse, diags diag.Diagnostics) diag.Diagnostics {
