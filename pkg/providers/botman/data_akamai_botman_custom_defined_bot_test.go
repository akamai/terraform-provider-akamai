		...
		expectedJSON := `
{
	"bots":[
		...
		client.BotMan.On("GetCustomDefinedBotList",
		...
		resource.UnitTest(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: testutils.LoadFixtureString(t, "testdata/TestDataCustomDefinedBot/basic.tf"),
		...
		expectedJSON := `
{
	"bots":[
		...
		resource.UnitTest(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: testutils.LoadFixtureString(t, "testdata/TestDataCustomDefinedBot/filter_by_id.tf"),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("data.akamai_botman_custom_defined_bot.test", "json", compactJSON(expectedJSON))),
				},
			},
