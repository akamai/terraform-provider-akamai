		...
		expectedJSON := `
{
	"customDenyActions":[
		...
		client.BotMan.On("GetCustomDenyActionList",
		...
		resource.UnitTest(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: testutils.LoadFixtureString(t, "testdata/TestDataCustomDenyAction/basic.tf"),
		...
		expectedJSON := `
{
	"customDenyActions":[
		...
		resource.UnitTest(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config: testutils.LoadFixtureString(t, "testdata/TestDataCustomDenyAction/filter_by_id.tf"),
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr("data.akamai_botman_custom_deny_action.test", "json", compactJSON(expectedJSON))),
				},
			},
