...
# This configuration is expected to fail because of missing required attribute.
# This behavior is related to current version of Terraform library and may change in the future.
...
resource "akamai_iam_api_client" "test" {
  ...
  api_access = {
    ...
    apis = [
      random_integer.choice.result == 1 ? {
        api_id       = 5580
...
resource "random_integer" "choice" {
  min = 0
  max = 1
}
