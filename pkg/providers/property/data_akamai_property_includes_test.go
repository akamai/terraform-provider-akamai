...
func TestDataPropertyIncludes(t *testing.T) {
	tests := map[string]struct {
		attrs      attributes
		init       func(*testing.T, *mockpapi, attributes)
		configPath string
		error      *regexp.Regexp
	}{
		"ListIncludes - without type filter": {
			...
			init: func(t *testing.T, m *mockpapi, attrs attributes) {
				mockListIncludes(m, attrs.contractID, attrs.groupID, attrs.includes, attrs.includesNumber, 5)
			},
		...
		"ParentProperty provided - ListAvailableIncludes - without type filter": {
			...
			init: func(t *testing.T, m *mockpapi, attrs attributes) {
				mockListAvailableIncludes(m, attrs.contractID, attrs.groupID, attrs.parentProperty.id, attrs.parentProperty.version, attrs.includesNumber, 5)
		...
		"missing required argument - contractID": {
			attrs:      attributes{},
			init:       func(t *testing.T, m *mockpapi, a attributes) {},
...
var (
	...
	mockListIncludes = func(m *mockpapi, contractID, groupID string, includes []papi.Include, includesNumber, timesToRun int) {
		m.On("ListIncludes", mock.Anything, papi.ListIncludesRequest{
			ContractID: contractID,
			GroupID:    groupID,
	...
	mockListAvailableIncludes = func(m *mockpapi, contractID, groupID, propertyID string, propertyVersion, includesNumber, timesToRun int) {
		m.On("ListAvailableIncludes", mock.Anything, papi.ListAvailableIncludesRequest{
			ContractID:      contractID,
			GroupID:         groupID,
	...
	mockGetInclude = func(m *mockpapi, contractID, groupID, includeID string, timesToRun int, include papi.Include) {
		m.On("GetInclude", mock.Anything, papi.GetIncludeRequest{
			ContractID: contractID,
			GroupID:    groupID,
			IncludeID:  includeID,
